...
我们提出了 **PD3QN**，一种新颖的增强型深度强化学习架构，旨在解决高动态、低容错环境（如 **带火球的 Flappy Bird**）中的**决策短视 (Decision Myopia)** 问题。

针对传统 Model-free 算法缺乏前瞻性的痛点，我们率先引入了 **轻量级一步状态预测器 (OSSP)** 来推断未来画面及潜在风险，并配合 **自适应置信度门控网络 (CGN)** 动态调节预测结果在决策中的权重。通过这种机制，PD3QN 能够在保证训练稳定性的同时，显著增强智能体的长程规划能力与鲁棒性。
